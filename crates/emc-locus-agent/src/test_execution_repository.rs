...
pub(crate) struct StoredSimulatedTestExecution {
    ...
    pub(crate) approved_template_id: Option<String>,
    pub(crate) approved_template_revision_id: Option<String>,
    pub(crate) approved_template_definition_checksum: Option<String>,
...
pub(crate) struct InsertSimulatedExecutionInput<'a> {
    ...
    pub(crate) approved_template_id: Option<&'a str>,
    pub(crate) approved_template_revision_id: Option<&'a str>,
    pub(crate) approved_template_definition_checksum: Option<&'a str>,
    ...
    transaction
        .execute(
            concat!(
                ...
                "(attempt_id, project_code, test_type, test_method_reference, ",
                "approved_template_id, approved_template_revision_id, approved_template_definition_checksum, ",
                "execution_mode, ",
            ...
            params![
                ...
                input.approved_template_revision_id,
                input.approved_template_definition_checksum,
...
pub(crate) fn load_simulated_execution(
    ...
    connection
        .query_row(
            concat!(
                "SELECT attempt_id, project_code, test_type, test_method_reference, ",
                "approved_template_id, approved_template_revision_id, approved_template_definition_checksum, ",
                "execution_mode, ",
                "operator, checked_on, status, readiness_ready, readiness_report_json, refusal_json, ",
                "instrumentation_snapshot_json, simulation_result_json, software_version, ",
...
fn stored_execution_from_row(
    ...
    Ok(StoredSimulatedTestExecution {
        ...
        approved_template_id: row.get(4)?,
        approved_template_revision_id: row.get(5)?,
        approved_template_definition_checksum: row.get(6)?,
        execution_mode: row.get(7)?,
        operator: row.get(8)?,
        checked_on: row.get(9)?,
        status: row.get(10)?,
        readiness_ready: row.get::<_, u8>(11)? == 1,
        readiness_report_json: row.get(12)?,
        refusal_json: row.get(13)?,
        instrumentation_snapshot_json: row.get(14)?,
        simulation_result_json: row.get(15)?,
        software_version: row.get(16)?,
