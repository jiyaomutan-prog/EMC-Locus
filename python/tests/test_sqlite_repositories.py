...
class ProjectRepositoryScheduleTests(unittest.TestCase):
    def _insert_corrupted_service_schedule_item(
        ...
        *,
        ...
        project_code: str,
        status: object,
        planned_start_at: object = "2026-07-01T09:00",
        planned_end_at: object = "2026-07-01T12:00",
        assigned_operator: object = "operator.one",
        location: object = "Lab A",
    ...
    def test_repository_rejects_non_text_service_schedule_status_on_insert(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            projects = ProjectRepository(
                Path(temporary_directory) / "projects.sqlite",
                Path("storage/sqlite"),
            ...
            projects.initialize()
            projects.create_project(
                code="CEM-REPO-STATUS-TEXT-INSERT",
                customer_name="Repository Schedule Customer",
                execution_mode="accredited",
                stage="test_planning",
            )
            ...
            with self.assertRaisesRegex(ValueError, "status must be text"):
                projects.add_service_schedule_item(
                    item_code="PLAN-REPO-STATUS-TEXT-INSERT",
                    project_code="CEM-REPO-STATUS-TEXT-INSERT",
                    title="Non-text status insert guard",
                    planned_start_at="2026-07-01T09:00",
                    planned_end_at="2026-07-01T12:00",
                    assigned_operator="operator.one",
                    location="Lab A",
                    equipment_under_test="EUT rail",
                    status=sqlite3.Binary(b"planned"),
                )

            self.assertEqual(projects.list_service_schedule_items(), [])

    def test_repository_rejects_non_text_service_schedule_status_on_audit_insert(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            projects = ProjectRepository(
            ...
            projects.initialize()
            projects.create_project(
                code="CEM-REPO-STATUS-TEXT-AUDIT-INSERT",
                customer_name="Repository Schedule Customer",
                execution_mode="accredited",
                stage="test_planning",
            )
            ...
            with self.assertRaisesRegex(ValueError, "status must be text"):
                projects.add_service_schedule_item_with_audit(
                    item_code="PLAN-REPO-STATUS-TEXT-AUDIT-INSERT",
                    project_code="CEM-REPO-STATUS-TEXT-AUDIT-INSERT",
                    title="Non-text audited status insert guard",
                    planned_start_at="2026-07-01T09:00",
                    planned_end_at="2026-07-01T12:00",
                    assigned_operator="operator.one",
                    location="Lab A",
            ...
            with closing(projects.connect()) as connection:
                schedule_count = connection.execute(
                    ...
                    WHERE project_code = ?
                    """,
                    ("CEM-REPO-STATUS-TEXT-AUDIT-INSERT",),
                ).fetchone()["schedule_count"]
                audit_count = connection.execute(
                    """
                    SELECT COUNT(*) AS audit_count
    ...
    def test_repository_rejects_normalized_duplicate_service_schedule_item_code_on_insert(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            projects = ProjectRepository(
            ...
            projects.initialize()
            projects.create_project(
                code="CEM-REPO-DUPLICATE-ITEM-NORM",
                customer_name="Repository Schedule Customer",
                execution_mode="accredited",
                stage="test_planning",
            )
            self._insert_corrupted_service_schedule_item(
                projects,
                item_code=" PLAN-REPO-DUPLICATE-ITEM-NORM ",
                project_code="CEM-REPO-DUPLICATE-ITEM-NORM",
                status="planned",
            )
            ...
            with self.assertRaisesRegex(
                ...
                projects.add_service_schedule_item(
                    item_code="PLAN-REPO-DUPLICATE-ITEM-NORM",
                    project_code="CEM-REPO-DUPLICATE-ITEM-NORM",
                    title="Normalized duplicate guard",
                    planned_start_at="2026-07-01T13:00",
                    planned_end_at="2026-07-01T15:00",
                    assigned_operator="operator.two",
                    location="Lab B",
    ...
    def test_repository_revalidates_imported_overlap_status_before_rejecting(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            projects = ProjectRepository(
            ...
            self._insert_corrupted_service_schedule_item(
                projects,
                item_code="PLAN-REPO-OVERLAP-STATUS-1",
                project_code="CEM-REPO-OVERLAP-STATUS",
                status="paused",
                planned_start_at="2026-07-01T09:00",
                planned_end_at="2026-07-01T12:00",
                assigned_operator="operator.one",
            ...
            with self.assertRaisesRegex(
                ValueError,
                "unknown service schedule status: paused",
            ):
                projects.add_service_schedule_item(
                    item_code="PLAN-REPO-OVERLAP-STATUS-2",
                    project_code="CEM-REPO-OVERLAP-STATUS",
                    title="Second operator slot",
                    planned_start_at="2026-07-01T11:00",
                    planned_end_at="2026-07-01T13:00",
            ...
            with closing(projects.connect()) as connection:
                schedule_count = connection.execute(
                    ...
                    WHERE project_code = ?
                    """,
                    ("CEM-REPO-OVERLAP-STATUS",),
                ).fetchone()["schedule_count"]
            self.assertEqual(schedule_count, 1)

    def test_repository_revalidates_non_text_imported_overlap_status(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            projects = ProjectRepository(
            ...
            projects.initialize()
            projects.create_project(
                code="CEM-REPO-OVERLAP-NON-TEXT-STATUS",
                customer_name="Repository Schedule Customer",
                execution_mode="accredited",
                stage="test_planning",
            )
            self._insert_corrupted_service_schedule_item(
                projects,
                item_code="PLAN-REPO-OVERLAP-NON-TEXT-STATUS-1",
                project_code="CEM-REPO-OVERLAP-NON-TEXT-STATUS",
                status=sqlite3.Binary(b"planned"),
                planned_start_at="2026-07-01T09:00",
                planned_end_at="2026-07-01T12:00",
                assigned_operator="operator.one",
            ...
            with self.assertRaisesRegex(
                ...
                projects.add_service_schedule_item(
                    item_code="PLAN-REPO-OVERLAP-NON-TEXT-STATUS-2",
                    project_code="CEM-REPO-OVERLAP-NON-TEXT-STATUS",
                    title="Second operator slot",
                    planned_start_at="2026-07-01T11:00",
                    planned_end_at="2026-07-01T13:00",
            ...
            with closing(projects.connect()) as connection:
                schedule_count = connection.execute(
                    ...
                    WHERE project_code = ?
                    """,
                    ("CEM-REPO-OVERLAP-NON-TEXT-STATUS",),
                ).fetchone()["schedule_count"]
            self.assertEqual(schedule_count, 1)
    ...
    def test_repository_rejects_non_text_service_schedule_target_status_on_update(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            projects = ProjectRepository(
            ...
            projects.initialize()
            projects.create_project(
                code="CEM-REPO-STATUS-TEXT-UPDATE",
                customer_name="Repository Schedule Customer",
                execution_mode="accredited",
                stage="test_planning",
            )
            projects.add_service_schedule_item(
                item_code="PLAN-REPO-STATUS-TEXT-UPDATE",
                project_code="CEM-REPO-STATUS-TEXT-UPDATE",
                title="Non-text target status update guard",
                planned_start_at="2026-07-01T09:00",
                planned_end_at="2026-07-01T12:00",
                assigned_operator="operator.one",
                location="Lab A",
            ...
            with self.assertRaisesRegex(ValueError, "status must be text"):
                projects.update_service_schedule_status(
                    item_code="PLAN-REPO-STATUS-TEXT-UPDATE",
                    status=sqlite3.Binary(b"confirmed"),
                )
    ...
    def test_repository_matches_normalized_service_schedule_project_on_update(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            projects = ProjectRepository(
            ...
            projects.initialize()
            projects.create_project(
                code="CEM-REPO-STATUS-PROJECT-NORM",
                customer_name="Repository Schedule Customer",
                execution_mode="accredited",
                stage="test_planning",
            )
            with closing(sqlite3.connect(projects.database_path)) as connection:
                ...
                connection.execute(
                    ...
                    """,
                    (
                        "PLAN-REPO-STATUS-PROJECT-NORM",
                        " CEM-REPO-STATUS-PROJECT-NORM ",
                        "Corrupted project code status update",
                        "2026-07-01T09:00",
                        "2026-07-01T12:00",
                        "operator.one",
                        "Lab A",
            ...
            projects.update_service_schedule_status(
                item_code="PLAN-REPO-STATUS-PROJECT-NORM",
                status="confirmed",
            )

            schedule = projects.list_service_schedule_items(
                project_code="CEM-REPO-STATUS-PROJECT-NORM",
            )
            self.assertEqual(len(schedule), 1)
            self.assertEqual(
                schedule[0]["project_code"],
                "CEM-REPO-STATUS-PROJECT-NORM",
            )
            self.assertEqual(schedule[0]["status"], "confirmed")
    ...
    def test_repository_rejects_non_text_service_schedule_status_on_update(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self._insert_corrupted_service_schedule_item(
                projects,
                item_code="PLAN-REPO-STATUS-BLOB",
                project_code="CEM-REPO-STATUS-BLOB",
                status=sqlite3.Binary(b"planned"),
            )
    ...
    def test_repository_rejects_invalid_service_schedule_block_on_update(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            self._insert_corrupted_service_schedule_item(
                projects,
                item_code="PLAN-REPO-STATUS-BLOCK",
                project_code="CEM-REPO-STATUS-BLOCK",
                status="planned",
                planned_start_at="2026-07-04T09:00",
                planned_end_at="2026-07-04T12:00",
            )
    ...
    def test_repository_matches_normalized_service_schedule_project_on_audit_update(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            projects = ProjectRepository(
            ...
            projects.initialize()
            projects.create_project(
                code="CEM-REPO-STATUS-AUDIT-PROJECT-NORM",
                customer_name="Repository Schedule Customer",
                execution_mode="accredited",
                stage="test_planning",
            )
            with closing(sqlite3.connect(projects.database_path)) as connection:
                ...
                connection.execute(
                    ...
                    """,
                    (
                        "PLAN-REPO-STATUS-AUDIT-PROJECT-NORM",
                        " CEM-REPO-STATUS-AUDIT-PROJECT-NORM ",
                        "Corrupted audit project code status update",
                        "2026-07-01T09:00",
                        "2026-07-01T12:00",
                        "operator.one",
                        "Lab A",
            ...
            audit_sequence = projects.update_service_schedule_status_with_audit(
                item_code="PLAN-REPO-STATUS-AUDIT-PROJECT-NORM",
                status="in_progress",
                actor="operator.two",
                reason="Start laboratory slot",
            )
            ...
            schedule = projects.list_service_schedule_items(
                project_code="CEM-REPO-STATUS-AUDIT-PROJECT-NORM",
            )
            events = projects.audit_events("CEM-REPO-STATUS-AUDIT-PROJECT-NORM")
            payload = json.loads(events[0]["payload_json"])
    ...
    def test_repository_rejects_non_text_service_schedule_status_on_audit_update(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            projects = ProjectRepository(
            ...
            projects.initialize()
            projects.create_project(
                code="CEM-REPO-STATUS-AUDIT-BLOB",
                customer_name="Repository Schedule Customer",
                execution_mode="accredited",
                stage="test_planning",
            )
            self._insert_corrupted_service_schedule_item(
                projects,
                item_code="PLAN-REPO-STATUS-AUDIT-BLOB",
                project_code="CEM-REPO-STATUS-AUDIT-BLOB",
                status=sqlite3.Binary(b"confirmed"),
            )
            ...
            events = projects.audit_events("CEM-REPO-STATUS-AUDIT-BLOB")
            with closing(projects.connect()) as connection:
                row = connection.execute(
                    """
                    SELECT status
    ...
    def test_repository_rejects_non_text_service_schedule_target_status_on_audit_update(
        self,
    ) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            projects = ProjectRepository(
            ...
            projects.initialize()
            projects.create_project(
                code="CEM-REPO-STATUS-AUDIT-TEXT-UPDATE",
                customer_name="Repository Schedule Customer",
                execution_mode="accredited",
                stage="test_planning",
            )
            projects.add_service_schedule_item(
                item_code="PLAN-REPO-STATUS-AUDIT-TEXT-UPDATE",
                project_code="CEM-REPO-STATUS-AUDIT-TEXT-UPDATE",
                title="Non-text audited target status update guard",
                planned_start_at="2026-07-01T09:00",
                planned_end_at="2026-07-01T12:00",
                assigned_operator="operator.one",
                location="Lab A",
            ...
            with self.assertRaisesRegex(ValueError, "status must be text"):
                projects.update_service_schedule_status_with_audit(
                    ...
                    status=sqlite3.Binary(b"confirmed"),
                    actor="operator.two",
                    reason="Advance with corrupted target status",
                )
    ...
    def test_repository_rejects_invalid_service_schedule_block_on_audit_update(
        ...
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            projects.initialize()
            projects.create_project(
                code="CEM-REPO-STATUS-AUDIT-BLOCK",
                customer_name="Repository Schedule Customer",
                execution_mode="accredited",
                stage="test_planning",
            )
            self._insert_corrupted_service_schedule_item(
                projects,
                item_code="PLAN-REPO-STATUS-AUDIT-BLOCK",
                project_code="CEM-REPO-STATUS-AUDIT-BLOCK",
                status="confirmed",
                planned_start_at="2026-07-04T09:00",
                planned_end_at="2026-07-04T12:00",
            )
            ...
            events = projects.audit_events("CEM-REPO-STATUS-AUDIT-BLOCK")
            with closing(projects.connect()) as connection:
                row = connection.execute(
                    """
                    SELECT status
    ...
    def test_repository_rejects_malformed_service_schedule_list_filters(self) -> None:
        with tempfile.TemporaryDirectory() as temporary_directory:
            ...
            with self.assertRaisesRegex(ValueError, "status must be text"):
                projects.list_service_schedule_items(status=sqlite3.Binary(b"planned"))
