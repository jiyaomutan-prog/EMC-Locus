...
function ProcedureTree(props: { nodes: import("../../models/methodWorkflow").ProcedureNode[]; depth?: number }) {
  return <ol className="procedureTree">{props.nodes.map((node) => <li key={node.node_id} style={{ marginLeft: (props.depth ?? 0) * 16 }}><span className="procedureKind">{node.node_kind.replaceAll("_", " ")}</span><strong>{node.label}</strong>{node.maximum_iterations && <small>Maximum {node.maximum_iterations} itérations</small>}{node.children.length > 0 && <ProcedureTree nodes={node.children} depth={(props.depth ?? 0) + 1} />}</li>)}</ol>;
}
