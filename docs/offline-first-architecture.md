...
When conflict detection reads existing local/reference snapshots, the Python
adapter revalidates both stored checksums before comparison to reject corrupted
or constraint-bypassed snapshot evidence.
